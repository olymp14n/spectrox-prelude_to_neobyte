...
-- script:  lua
-- references:
--  [1] https://github.com/mlepage/heightmap
--  [2] http://en.wikipedia.org/wiki/Diamond-square_algorithm
...
function core_heightmap_normalize(map)
    local max = -math.huge
    local min = math.huge
...
function core_heightmap_create(width, height, f)
